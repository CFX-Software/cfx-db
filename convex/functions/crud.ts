...
export const execute = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            await writeAuditLog(ctx, auditLog);
        }
    }
});
...
const MAX_BATCH_SIZE = 500;   // Increased batch size for better throughput
...
async function writeAuditLog(ctx: any, log: {
    ...
    try {
        ...
        if (log.playerId !== undefined) payload.playerId = log.playerId;
        if (log.data !== undefined) payload.data = log.data;
        if (log.where !== undefined) payload.where = log.where;
        if (log.results !== undefined) payload.results = log.results;
        if (log.error !== undefined) payload.error = log.error;

        await ctx.db.insert("auditLogs", payload);
    } catch (error: any) {
...
/**
 * INSERT: Create new record(s) - OPTIMIZED
 * Uses parallel processing for batch inserts
 */
async function handleInsert(ctx: any, table: string, data: any) {
    ...
    // Handle batch insert with parallel processing
    if (Array.isArray(data)) {
        if (data.length > MAX_BATCH_SIZE) {
            throw new Error(`Batch size ${data.length} exceeds maximum ${MAX_BATCH_SIZE}`);
        }

        // Pre-validate and sanitize all data first
        ...
        // Insert all records in parallel for maximum throughput
        const insertPromises = preparedData.map(sanitized =>
            ctx.db.insert(table as any, sanitized)
        );
...
/**
 * UPDATE: Modify existing record(s) - OPTIMIZED
 * Uses parallel processing for batch updates.
 */
async function handleUpdate(ctx: any, table: string, where: any, data: any) {
    ...
    let query = ctx.db.query(table);
    query = applyWhereClause(query, where);
    const records = await query.take(MAX_COLLECT);

    if (records.length >= MAX_COLLECT) {
        console.warn(`[cfx-db] Update matched ${MAX_COLLECT}+ records, capping at ${MAX_COLLECT}`);
    }
    ...
    // Pre-validate data once
    const sanitized = sanitizeData(data);
    validateData(table, sanitized, "update");

    // Update in chunks to avoid overwhelming Convex with large fanout.
    const CHUNK_SIZE = 100;
    for (let i = 0; i < records.length; i += CHUNK_SIZE) {
        const chunk = records.slice(i, i + CHUNK_SIZE);
        await Promise.all(chunk.map((record: any) => ctx.db.patch(record._id, sanitized)));
    }
...
/**
 * DELETE: Remove record(s) - OPTIMIZED
 * Uses parallel processing for batch deletes.
 */
async function handleDelete(ctx: any, table: string, where: any) {
    ...
    if (records.length >= MAX_COLLECT) {
        console.warn(`[cfx-db] Delete matched ${MAX_COLLECT}+ records, capping at ${MAX_COLLECT}`);
    }
    ...
    // Delete in chunks to avoid overwhelming Convex with large fanout.
    const CHUNK_SIZE = 100;
    for (let i = 0; i < records.length; i += CHUNK_SIZE) {
        const chunk = records.slice(i, i + CHUNK_SIZE);
        await Promise.all(chunk.map((record: any) => ctx.db.delete(record._id)));
    }
 ...
 * Supports: =, !=, >, <, >=, <=, AND, OR
 *
 * NOTE: For optimal performance, ensure your schema has indexes on frequently
 * queried fields. For dedicated high-performance queries, create custom
 * query functions that use .withIndex() directly.
 */
function applyWhereClause(query: any, where: any): any {
    ...
    // Use filter for all conditions (Convex optimizes this internally)
    return query.filter((q: any) => {
        let result: any = null;
...
/**
 * Batch insert helper (for insertMany) - OPTIMIZED
...
export const insertMany = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Pre-validate and sanitize all data first
        const preparedRecords = args.records.map(record => {
        ...
        const insertPromises = preparedRecords.map(sanitized =>
            ctx.db.insert(args.table as any, sanitized)
        );
...
/**
 * Batch update helper (for updateMany with different values) - OPTIMIZED
 * Uses parallel processing for maximum throughput
 */
export const updateMany = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const updatePromises = args.updates.map(update =>
            handleUpdate(ctx, args.table, update.where, update.set)
        );
...
/**
 * Batch delete helper (for deleteMany with different where clauses) - OPTIMIZED
 * Uses parallel processing for maximum throughput
 */
export const bulkDelete = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const deletePromises = args.whereList.map(where =>
            handleDelete(ctx, args.table, where)
        );
...
/**
 * Batch upsert helper (for upsertMany) - OPTIMIZED
 * Uses parallel processing for maximum throughput
 */
export const bulkUpsert = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const upsertPromises = args.operations.map(op =>
            handleUpsert(ctx, args.table, op.where, op.insert, op.update)
        );
