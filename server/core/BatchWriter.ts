...
export class BatchWriter {
    ...
    async queueWrite<T>(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (queue.length >= this.MAX_QUEUE_SIZE) {
                Logger.getInstance().scoped("batch").warn("Queue overflow, dropping oldest", { priority });
     ...
     * Optimized for low latency while still batching
     */
    private getFlushInterval(priority: Priority): number {
        switch (priority) {
            case "high":
                return 5; // 5ms - near-instant for high priority
            case "normal":
                return Math.min(this.config.batchFlushIntervalMs, 25); // Cap at 25ms for responsiveness
            case "low":
                return 100; // Reduced from 200ms
            default:
                return 25;
        }
    }
    ...
    private async flush(priority: Priority): Promise<void> {
        ...
        // Execute mutations with controlled concurrency based on Convex plan
        // IMPORTANT: Convex limits concurrent mutations:
        // - Free/Starter: 16 concurrent → use 12
        // - Professional: 256 concurrent → use 50
        const MAX_CONCURRENT = this.config.convexPlan === "professional" ? 50 : 12;

        // Process in chunks to respect Convex concurrency limits
    ...
    dispose(): void {
        ...
        Logger.getInstance().scoped("batch").debug("BatchWriter disposed");
    }
}
