...
export class SubscriptionManager {
    ...
    static getInstance(): SubscriptionManager {
        if (!SubscriptionManager.instance || SubscriptionManager.instance.isDisposed) {
    ...
    static hasInstance(): boolean {
        return SubscriptionManager.instance !== null && !SubscriptionManager.instance.isDisposed;
    }
    ...
    private handleUpdate(subscriptionId: string, data: any): void {
        ...
        for (const callback of sub.subscribers.values()) {
            try {
                ...
                Logger.getInstance().scoped("subscription").error("Error in subscription callback", { error: String(error) });
            }
        }
    }
    ...
    private getSubscriptionKey(query: FunctionReference<"query">, args: any): string {
        return `${query.toString()}:${stableStringify(args ?? {})}`;
    }
    ...
    dispose(): void {
        ...
        Logger.getInstance().scoped("subscription").debug("SubscriptionManager disposed");
    }
}
