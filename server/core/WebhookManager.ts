...
export class WebhookManager {
    ...
    configure(webhooks: WebhookConfig[]): void {
        ...
        for (const webhook of webhooks) {
            if (!this.isValidWebhookURL(webhook.url)) {
                console.error(`[cfx-db] Invalid webhook URL: ${webhook.url}`);
                continue;
            }

            if (!webhook.events || webhook.events.length === 0) {
                console.error(`[cfx-db] Webhook must specify at least one event`);
                continue;
            }
        ...
        console.log(`[cfx-db] Registered ${this.webhooks.length} webhooks`);
    }
    ...
    private isValidWebhookURL(url: string): boolean {
        try {
            ...
            if (parsed.protocol !== "https:") {
                console.error(`[cfx-db] Webhook URL must use HTTPS: ${url}`);
                return false;
            }
            ...
            if (
                ...
                console.error(`[cfx-db] Webhook URL cannot be localhost or internal IP: ${url}`);
                return false;
            }

            return true;
        } catch (error) {
            console.error(`[cfx-db] Invalid webhook URL format: ${url}`);
            return false;
        }
    }
    ...
    async trigger(payload: WebhookPayload): Promise<void> {
        ...
        for (const webhook of matchingWebhooks) {
            this.deliver(webhook, payload).catch((error) => {
                console.error(
                    `[cfx-db] Webhook delivery failed for ${webhook.url}:`,
                    error.message
                );
            });
        }
    ...
    private async deliver(webhook: WebhookConfig, payload: WebhookPayload): Promise<void> {
        ...
        try {
            ...
            console.log(`[cfx-db] Webhook delivered successfully: ${payload.event} -> ${webhook.url.slice(0, 50)}...`);
        } catch (error: any) {
            console.error(
                `[cfx-db] Webhook delivery failed for ${webhook.url} (event: ${payload.event}):`,
                error.message
            );
            ...
            if (webhook.retryOnFailure) {
                ...
                if (attempts < (webhook.maxRetries || this.MAX_RETRIES)) {
                    ...
                    console.log(
                        `[cfx-db] Retrying webhook delivery (attempt ${attempts + 1}/${webhook.maxRetries || this.MAX_RETRIES})...`
                    );
                    ...
                    console.error(
                        `[cfx-db] Webhook delivery failed after ${attempts} retries. Giving up.`
                    );
                    this.retryAttempts.delete(webhookId);
                }
            }
