...
export class ConnectionManager {
    ...
    async initialize(): Promise<void> {
        ...
        const poolSize = Math.min(this.config.connectionPoolSize, maxPoolSize);
        console.log(`[cfx-db] Initializing connection pool (size: ${poolSize}, plan: ${this.config.convexPlan})`);
        ...
        console.log("[cfx-db] Connection manager initialized and warmed up");
    }
    ...
    private async warmupConnections(): Promise<void> {
        console.log("[cfx-db] Warming up connections...");
        ...
        try {
            ...
            console.log(`[cfx-db] ✓ Connection warmup completed in ${warmupTime}ms`);
        } catch (error) {
            console.warn("[cfx-db] Connection warmup partially failed (non-fatal)");
        }
    }
    ...
    private async performHealthCheck(): Promise<boolean> {
        if (this.isDisposed || this.httpClientPool.length === 0) {
            console.log("[cfx-db] Health check skipped: disposed or no pool");
            return false;
        }
        ...
        try {
            ...
            console.log("[cfx-db] Performing health check query...");
            ...
            console.log("[cfx-db] Health check query succeeded:", result ? "got data" : "no data");
            ...
            console.error("[cfx-db] Error details:", {
            ...
            if (wasHealthy) {
                console.error("[cfx-db] ✗ Connection lost");
            }
