...
// Track subscriptions by player source for cleanup on disconnect
const playerSubscriptions = new Map<number, Set<string>>();
...
export async function luaMutation(
    ...
    try {
        const mutationOptions = priority ? { priority } : {};
        const result = await mutation(mutationName as any, args, mutationOptions);

        // Invalidate cache for this table (mutations change data)
        if (args && args.table) {
            const invalidated = queryCache.invalidateTable(args.table);
            if (invalidated > 0) {
                Logger.getInstance().scoped("cache").debug("Cache entries invalidated", { table: args.table, count: invalidated });
            }
        }
        ...
        if (args && args.table && args.operation) {
            ...
            webhookManager.trigger(eventData).catch((err) => {
                Logger.getInstance().scoped("webhook").error("Webhook trigger error", { error: String(err) });
            });
        }
...
export function luaSubscribe(
    ...
    // Track by player source when available (player-triggered context)
    const source = Number((global as any).source || 0);
    if (Number.isFinite(source) && source > 0) {
        if (!playerSubscriptions.has(source)) {
            playerSubscriptions.set(source, new Set());
        }
        playerSubscriptions.get(source)!.add(subscriptionId);
    }
...
export function luaUnsubscribe(subscriptionId: string): boolean {
    ...
    for (const [source, subs] of playerSubscriptions.entries()) {
        if (subs.has(subscriptionId)) {
            subs.delete(subscriptionId);
            if (subs.size === 0) {
                playerSubscriptions.delete(source);
            }
            break;
        }
    }
...
/**
 * Cleanup all subscriptions for a player source
 */
function cleanupPlayerSubscriptions(source: number): void {
    const subs = playerSubscriptions.get(source);
    if (!subs) return;

    let cleaned = 0;
    for (const subId of Array.from(subs)) {
        if (luaUnsubscribe(subId)) {
            cleaned++;
        }
    }
    ...
    if (cleaned > 0) {
        Logger.getInstance().scoped("subscription").debug("Cleaned up player subscriptions", {
            source,
            count: cleaned,
        });
    }
...
function cleanupResourceSubscriptions(resourceName: string): void {
    ...
    if (cleaned > 0) {
        Logger.getInstance().scoped("subscription").debug("Cleaned up subscriptions", { resource: resourceName, count: cleaned });
    }
}
...
export async function executeTransaction(
    ...
    try {
        ...
        Logger.getInstance().scoped("transaction").error("Transaction failed", { error: error.message });
...
export function addValidationRules(table: string, rules: any): void {
    ...
    Logger.getInstance().scoped("validation").warn("addValidationRules is not yet implemented - validation rules must be added in convex/lib/validation_rules.ts");
}
...
// Cleanup player-scoped subscriptions on disconnect
on("playerDropped", () => {
    const source = Number((global as any).source || 0);
    if (Number.isFinite(source) && source > 0) {
        cleanupPlayerSubscriptions(source);
    }
});
