...
function DB.insertMany(table, records, callback)
    if callback then
        exports['cfx-db']:_luaMutation('functions/crud:insertMany', {
            table = table,
            records = records
...
function DB.updateMany(table, updates, callback)
    if callback then
        exports['cfx-db']:_luaMutation('functions/crud:updateMany', {
            table = table,
            updates = updates
        }, function(success, result)
            if success then
...
function DB.upsert(table, options, callback)
    ...
    if callback then
        exports['cfx-db']:_luaMutation('functions/crud:execute', {
            operation = 'upsert',
            table = table,
            where = options.where,
...
function DB.execute(sql, params, callback)
    print("^3[cfx-db] WARNING: DB.execute() is deprecated and unreliable.^7")
    print("^3[cfx-db] Use DB.insert(), DB.select(), DB.update(), DB.delete() instead for better reliability.^7")
...
function DB.scalar(sql, params, callback)
    print("^3[cfx-db] WARNING: DB.scalar() is deprecated.^7")
    print("^3[cfx-db] Use DB.count() or DB.selectOne() instead.^7")
    if callback then
...
function DB.subscribe(table, options, callback)
    ...
    local subId = exports['cfx-db']:_luaSubscribe('functions/crud:read', payload, callback)

    -- Return unsubscribe function
    return function()
        exports['cfx-db']:_luaUnsubscribe(subId)
    end
end
...
function executeOperation(operation, table, options, callback)
    ...
    if callback then
        exports['cfx-db']:_luaMutation('functions/crud:execute', payload, function(success, result)
            if success then
                callback(result)
            else
                callback(nil, result)
